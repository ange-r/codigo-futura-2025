...
/// Trait que define la interfaz del token segÃºn CAP-46
/// 
/// Esta es la interfaz estÃ¡ndar de tokens fungibles en Stellar
...
pub trait TokenTrait {
    ...
    /// - Decimals: precisiÃ³n del token (7 para Stellar)
    fn initialize(
        env: Env, 
        admin: Address, 
        name: String, 
    ...
    /// 
    /// Requiere autorizaciÃ³n de `from`
    /// No permite transferencias a sÃ­ mismo
    fn transfer(
        env: Env, 
        from: Address, 
        to: Address, 
    ...
    /// 
    /// Devuelve cuÃ¡nto puede gastar `spender` de los tokens de `from`
    fn allowance(env: Env, from: Address, spender: Address) -> i128;
    
    /// Transfiere tokens en nombre de otro usuario
    /// 
    /// Requiere allowance previo mediante approve()
    /// Reduce el allowance automÃ¡ticamente
    fn transfer_from(
        env: Env, 
        spender: Address, 
        from: Address, 
...
impl TokenTrait for TokenBDB {
    fn initialize(
        ...
        // 1. Verificar que no estÃ© inicializado
        if env.storage().instance().has(&DataKey::Initialized) {
            return Err(TokenError::AlreadyInitialized);
        }
        
        // 2. Validar decimales (mÃ¡ximo 18 como Ethereum)
        if decimals > MAX_DECIMALS {
            return Err(TokenError::InvalidDecimals);
        }
        
        // 3. Validar metadatos (name y symbol no vacÃ­os)
        ...
        // 5. Extender TTL del storage de instance (30 dÃ­as)
    ...
    fn mint(env: Env, to: Address, amount: i128) -> Result<(), TokenError> {
        ...
        // 4. Validar que `to` no sea igual a `admin` (opcional, pero buena prÃ¡ctica)
        // Esto evita que el admin se mintee tokens a sÃ­ mismo por error
    ...
    fn burn(env: Env, from: Address, amount: i128) -> Result<(), TokenError> {
        ...
        // 2. Requiere autorizaciÃ³n del dueÃ±o de los tokens
        from.require_auth();
    ...
    fn transfer(
        ...
        // 2. Verificar autorizaciÃ³n del sender
        from.require_auth();
        ...
        // 4. No permitir transferencia a sÃ­ mismo (gas-efficient)
        if from == to {
        ...
        // 5. Calcular nuevos balances con verificaciÃ³n de overflow
        ...
        // OptimizaciÃ³n: si from_balance = 0, eliminar key
    ...
    fn approve(
        ...
        // 2. Verificar autorizaciÃ³n del owner
        from.require_auth();
        
        // 3. ValidaciÃ³n: amount debe ser >= 0 (permitir 0 para revocar)
        if amount < 0 {
            return Err(TokenError::InvalidAmount);
        }
        ...
        if amount == 0 {
            // OptimizaciÃ³n: eliminar key si allowance = 0
    ...
    fn transfer_from(
        ...
        // 2. Verificar autorizaciÃ³n del spender
        ...
        // 4. No permitir transferencia a sÃ­ mismo
        if to == from {
            return Err(TokenError::InvalidRecipient);
        }
        ...
        // 8. Actualizar estado atÃ³micamente
        // OptimizaciÃ³n: eliminar keys si son 0
        if new_from_balance == 0 {
            env.storage().persistent().remove(&DataKey::Balance(from.clone()));
        } else {
            env.storage().persistent().set(
    ...
    fn name(env: Env) -> String {
        // Verificar inicializaciÃ³n antes de devolver metadata
        if !env.storage().instance().has(&DataKey::Initialized) {
